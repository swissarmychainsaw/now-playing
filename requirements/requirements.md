...
AI Prompt Example: Build a React component called SearchBar using Tailwind.
...
Includes: Header, SearchBar, RecommendationTabs, MovieCard grid
Initial behavior:
...
- Divided into three sections:
  ...
  3. Director & top three cast list  (right); with images. Images when hovered enlarge. Make two data sources in case on does not deliver an image. 
  4. Under Director & cast list will be: "watch tailer" (also has at least two data sources), and "Where to watch" also with multi data options
- Also includes a RecommendedMoviesSection displayed directly below the main card

### MovieRating (Reusable)

- Appear prominently at the top of the Card, and is the main "call to action"
- Do not use white font on white backgrounds
...
## 🎬 Movie Card Types Clarification

There are two distinct types of movie cards used in the Search Results and Landing page flows:

### 1. PrimaryMovieCard

- **Purpose:** Displays the main result from a user-initiated search (e.g., when searching for “Star Wars”).
- **Placement:** Appears at the top of the `SearchResults` page.
- **Content Includes:**
  - Poster, title, metadata
  - Full overview (not truncated)
  - Rating controls (stars, watchlist, not interested)
  - Cast preview and streaming options
- **Design Priority:** Serves as the anchor for related recommendations.

### 2. RecommendationMovieCard

- **Purpose:** Represents one of the five recommendations based on the PrimaryMovieCard (e.g., by actor, director, genre).
- **Placement:** Appears beneath the PrimaryMovieCard.
- **Content Includes:**
  - Poster, title, release year
  - Actor/director tag that links it to the primary movie
  - Optional rating controls (if included)
- **Behavior:** Clicking any of these re-centers the search around that movie, replacing the PrimaryMovieCard and regenerating recommendations.

Certainly. Here's a Markdown snippet you can add under the **Header** or **App Shell** section of your `requirements.md` file to define the browser tab display behavior, including the favicon and page title:

````md
## 🌟 Browser Tab: Title and Favicon

### Page Title (Display Tab)
- The browser tab should display the title:  
  **Now Playing – Rate & Discover Movies**

- This helps reinforce brand identity and improves discoverability when multiple tabs are open.

### Favicon (Favorite Icon)
- A small favicon icon should appear in the browser tab.
- **Design Requirements:**
  - The icon should relate to movies (e.g., a film reel, clapperboard, popcorn, or a star)
  - Prefer SVG or high-res `.ico` format
  - Keep it minimal for visibility at small sizes (16x16, 32x32)

### Implementation Notes
- Add the following to `index.html` in the `<head>` section:

```html
<title>Now Playing – Rate & Discover Movies</title>
<link rel="icon" href="/favicon.ico" type="image/x-icon" />
````

* You may also include multiple favicon sizes for device support:
...
* Store favicon assets in `/public/` so they are copied to the build output.
